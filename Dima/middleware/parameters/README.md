...
完整协议目录保留只读 Fact。普通产品常量从单枚举/min=max 生成约束，持久化加载跳过覆盖；例如 EKF2_HGT_REF 仍只接受其唯一值。

`Firmware Observations` 是权威 YAML 中固件拥有的固定零默认观测组：正式生成器生成成员、只读属性及恢复策略。CAL_MAG_MOT_KX/KY/KZ/ID/GEN 使用该组，QGC 协议拒绝全部外部写入（包括零），固件事务可写非默认结果，快照会恢复其真实值。有效性由 ID/GEN 决定，零 K 可以有效。磁校正字段及板级细调的依赖边从权威字段族正式生成；参数核心在同一锁内清依赖 ID，各校准事务回滚时最后恢复观测快照。源字段和观测永远一起进入保存快照，不能用仅在 RAM 的失效标志。
