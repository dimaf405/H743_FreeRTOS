...
- **评估实体共享：** 六个 `evaluate_bank<Delay>` 只保留样本数门禁与系数/方差读取，再调用非模板 `evaluate_model`，避免重复展开评估公式。六个 RLS 状态、0～5 的比较顺序、平分选择、浮点精度、失败条件与 PI 计算均不变；没有通过减少模型数来减重。
- **RLS 更新实体共享：** `ArxRls<N,M,D>` 保留原状态布局、输入历史和缓冲填满条件；数学更新按 `N+M+1` 维度共享模板实体，不按 D 复制。采样前的旧系数快照与 P/innovation/theta/差值的表达式顺序保持，六个模型继续各自更新；没有切换精度、重排公式或引入动态分配。
