...
struct StateSample {
	...
	matrix::Vector<float, 22> Data() const {
		matrix::Vector<float, 22> state;
	...
	const matrix::Vector<float, 22>& vector() const {
		return *reinterpret_cast<matrix::Vector<float, 22>*>(const_cast<float*>(reinterpret_cast<const float*>(&quat_nominal)));
	};

};
static_assert(sizeof(matrix::Vector<float, 22>) == sizeof(StateSample), "state vector doesn't match StateSample size");
...
namespace State {
	...
	static constexpr uint8_t size{21};
};
}
#endif // !EKF_STATE_H
