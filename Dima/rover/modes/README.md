...
- **关联自动整定：** 基础阶段之后采集未零区化噪声、多档前进/CW/CCW 响应（无倒退），辨识前馈、运行参数及速度/yaw-rate 四项 P/I，再验证 Heading、转驱与 RAM 路径。32 槽事务保留最初快照；运行/FF/PI/导航候选只在 RAM，最终验证后整组保存，失败整组回滚。前进模式不试改 Manual 共用电机整形/SLEW，明确记入 skipped_stages。`revise_float/apply_revisions/finalize_provisional` 不借用磁专用 `refine`，不要求人工确认候选值。
...
- **长度精简：** `RO_CAL_DIST` 已退役，内部保持原默认 12 m 期望尺度；实际直线按 `min(12, R_work-distance-0.5)` 自动缩短，不足 5 m 拒绝。RAM 路径也不超过这一尺度，阶段/会话预算及现有安全配置不变；旧快照中的自定义期望长度不再生效。
- **速度与包络：** 入场正 RO_SPEED_LIM（默认 1.0）是唯一速度上限，0/-1 拒绝运动；E=入场冻结 MOT_THR_MAX，纵向请求/闭环输出 [0,E]、转向 [-1,1]、轮端 [-E,E]。无运动按时间继续升档，到 0.995E 后连续 8 s 无运动才报 FAILURE_DRIVE_ENVELOPE。校准无倒退，FWD=0→CW=1→CCW=2；前进顶档有效的后端输出达到 0.995E 才一次报告端点，不代表 RPM。
- **磁-油门补偿：** 高输出直线段按去/返程分别拟合原始磁场截距及公共斜率，使用后端已应用输出历史对齐；最终 K_body=R*S*K_sensor。基础磁校正保存后启动 COMMIT_MAG_MOT 独立事务，五项只读观测参数须前端确认并保存。STAGE_MAG_MOT 与基础磁阶段分开，补偿不可观测/回滚时报告 PARTIAL。前端只在两轮均非负且输出在磁样本之前 100 ms 内时应用 B-K*u；干扰率未知 -1，>30% 提示硬件整改。
...
- **响应子状态：** 每个 FWD/CW/CCW 分为 PROBE→RAMP→SETTLE→COLLECT→BRAKE。起步后保持请求并稳定 0.5 s；每档到位后连续稳定 1 s，收集至少 10 点且覆盖 1.2 s 才换档。窗口预算由实际 slew 推导，每个响应运动最多 90 s、保留最终 15 s 停车，全场仍不超过 600 s。CW/CCW 使用杆臂转动空间，不要求五米直线。
- **事务恢复与结果：** 磁事务 Done/Failed 均先同步自有参数计数；bootstrap 回滚可正常继续整定。完整的内环九段验证后转入 Heading；计数越界拒绝。SUCCESS 表示适用项目完成；支持但未完成仍是 PARTIAL，范围外跳过不冒充完成。
- **诊断与状态图：** 周期日志携带 session、skipped/unavailable、采集子状态和证据快照；重复终态不清空建议工具的端点/磁干扰证据。详细转移、超时和授权见 [当前状态机](../../../docs/AUTO_CALIBRATION_STATE_MACHINE_ZH.md)。
