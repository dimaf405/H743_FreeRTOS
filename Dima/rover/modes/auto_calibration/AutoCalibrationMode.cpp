...
bool AutoCalibrationMode::read_config() noexcept
{
    ...
    Config next{};
    const bool loaded = read(dima::params::RO_CAL_RADIUS, next.radius) && read(dima::params::RO_CAL_STOP_D, next.stop_distance) &&
        read(dima::params::RO_SPEED_LIM, next.entry_cruise) &&
