...
- Parameter 继续以 FlashFS 为主存储、SD 为镜像；Mission 改为 SD 优先、无卡只保留 RAM，不读取或写入任务 Flash。SYS_DM_BACKEND 从权威 YAML 移除。逐项上传进入固定 staging，完整任务在 SD 原子保存成功或 RAM 发布后才发送最终 ACK；SD 失败回退 RAM 并明确报告非持久状态。未解锁热插卡只同步当前已确认 RAM 任务，AUTO 运行 current 不写回；无卡重启为空任务。具体事务与日志边界见 `Dima/modules/mission/README.md`。
