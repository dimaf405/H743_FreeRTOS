...
## 当前实施状态（2026-09-14，ADR 0005）

本节取代 R1–R5 的逐段人工 Arm 和固定 1.5 m/s 操作说明。旧记录折叠保留，只作历史证据。

当前实现采用一次 Arm 会话授权、冻结 MOT_THR_MAX 包络、双层 GNSS 围栏、前进/CW/CCW 响应、统一 IMU 稳定零偏及关联 RAM 事务。磁补偿使用后端已应用输出历史和原始磁回归，基础磁校准与补偿分别确认、保存和报告。第 7 节 Windows 制品是历史证据；本轮 Linux 构建与工具冒烟单独记录，车辆与 QGC 验收仍待执行。

| 检查点 | 实现内容 | 验收状态 |
|---|---|---|
| U1 一次 Arm | Commander 独占 session 授权，内部 Disarm/正常 preflight/继续；外部 Disarm 在提交窗口也取消 | 源码定点审查；实板竞态待验收 |
| U2 冻结策略 | 入场正 RO_SPEED_LIM 为唯一速度上限，0/-1 拒绝运动；独立捕获固定圆和停车预算 | 正式生成与 Linux 原生构建通过；停车能力待实板证明 |
| U3 前进与观测 | FWD→CW→CCW、原始未零区反馈、生产限制器原因 | 校准纵向非负；停车及原地转向待验收 |
| U4 IMU/满输出 | 实际前端快照、EKF 稳定偏置、残差确认；前进顶档有效样本的命令端点证据 | 不测 RPM，不保证达到端点 |
| U5 关联整定 | 32 槽原始快照、运行/FF/PI/Heading/转驱/路径验证、自动保存/回滚；全局电机整形/SLEW 明确跳过 | 源码审查、正式生成、Linux 构建及工具冒烟通过；实际整定和掉电验收待执行 |

没有新编码器/电流反馈依赖，没有“确认增益”按钮或参数。代码、静态门禁和 Windows 构建不能替代 QGC、板卡、掉电持久化或车辆性能验收；本轮不刷机、不连接串口、不触发车辆动作。

本轮状态修复及每个状态的转移/超时见 [当前详细状态机](AUTO_CALIBRATION_STATE_MACHINE_ZH.md)。其中 skipped_stages 与 unavailable_stages 分别表达明确跳过和支持但未完成。

## 1. 操作流程与一次 Arm 授权

1. 车体放在已知水平基准，保持 Disarmed。设置真实轮距、PWM 功能/方向/端点，准备有效 RC、Kill 和符合当前载荷/地面的停车距离上界。
2. 设定 `RO_CAL_RADIUS`、`RO_CAL_STOP_D`；核对入场 `RO_SPEED_LIM`（默认 1.0，与导航巡航同源，为唯一会话速度上限）。停车能力未知时保持 `RO_CAL_STOP_D=0`，固件只允许可用静态项。
...
→ 前馈与 RTK 约束磁学习（磁可选）→ 磁基础校正确认/保存 → 磁补偿独立事务
→ EKF 稳定 IMU 零偏/前端与残差确认
→ 未零区化噪声、多档前进响应 → CW → CCW
→ 运行/FF/PI/导航关联 RAM 候选（保留电机整形原值）
...
| 参数 | 默认/范围 | 当前语义 |
|---|---|---|
| `RO_SPEED_LIM` | 默认 1.0；Metadata -1–100 m/s | 入场正值为唯一会话速度上限；非正值或非有限值拒绝动态校准 |
| `RO_CAL_RADIUS` | 20；1–100 m | GNSS 定位参考点的固定水平圆半径 |
| `RO_CAL_STOP_D` | 0；0–100 m | 请求停车到实际停稳的可信保守最大行驶距离；0 禁止动态校准 |
| `MOT_THR_MAX` | 入场冻结，0.05–1.0 | 纵向请求/闭环输出 [0,E]，转向请求 [-1,1]，最终每轮 [-E,E]，不自动提高 |

```text
RO_SPEED_LIM = 正值（默认 1.0）: V_session = 入场冻结值；0/-1 拒绝动态运动
```
...
所有前进直线、掉头、磁转动、闭环及 RAM 路径共用该圆。剩余空间不足提前停车/跳过；围栏介入数据不计辨识，不扩大半径、不自动返回圆内。越界、定位失效/设备切换撤销有效动力请求。

所有校准阶段每轮输出绝对值不超过入场冻结的 `MOT_THR_MAX`，最终变化率始终 ≤0.15/s；地速 ≤`V_session`、yaw-rate ≤0.6 rad/s、实际线/角加速度 ≤3 m/s²、3 rad/s²。每个运动周期 180 s、全会话 600 s 不变，等待、重融合、应用、保存也消耗总预算。

### 前进激励、弱动力分流与端点

- 激励依次为 FWD=0、CW=1、CCW=2，收尾哨兵为 3；原地转向允许左右轮反向，校准纵向请求始终非负。RTK 返程通过掉头前进完成。
- 无运动时按既有 0.15/s 请求 slew 继续爬升，记录本航段已确认起步的请求下界，不依赖已行驶距离解锁高档。请求达到 0.995E 后连续 8 s 仍无运动才报告 `FAILURE_DRIVE_ENVELOPE`，提示检查 MOT_THR_MIN、供电和机械。
- 前进顶档样本可用、无安全限制且后端已应用输出达到 `observed_output_max ≥ 0.995E` 时，一次性 Info 报告命令端点。方向异常、速度/定位/围栏守卫仍优先停车。
- 没有 RPM/编码器，不报告最大转速或堵转扭矩；命令端点不代替 PI 调节余量，PI 仅公式法，FF 余量不可行如实报告该组未完成。
...
| 前馈 | `RO_MAX_THR_SPEED`、`RO_YAW_RATE_CORR` | 真实多档前进与 CW/CCW 响应；不能让 PI 积分代替 FF 辨识 |
| 内环 | `RO_SPEED_P/I`、`RO_YAW_RATE_P/I` | 四项同代 RAM 应用，真实前进速度和 CW/CCW 角速度验收 |
...
| 等效电机整形 | `MOT_THR_MIN`、`MOT_THR_EXPO`、`MOT_THR_ASYM` | 当前前进模式跳过，保留原值；本模式没有全正反向证据，不写 Manual 共用整形 |
| 电机 slew | `MOT_SLEW_RATE` | 当前模式明确跳过并保留原值；已有 MOT slew 仍限制输出并纳入激励时间预算 |
...
- `MOT_THR_MIN/EXPO/ASYM/SLEW_RATE` 是 Manual 等消费者共用的全局值。当前前进数据不覆盖 Manual 的完整正反向使用域；前进顶档达端点也不授权全局整形修改。因此缺少全域证据时保留原值，不能把“有候选算法”说成每车都可自动完成。
- 特别是默认 `MOT_SLEW_RATE=1`，校准末端仍为 0.15/s，因此不能从这些数据识别默认电机 slew；前进端点也不能补齐缺失的反向整形证据。这些默认条件下明确不自动写全局整形/SLEW，不绕过安全层制造观测。全局整形/SLEW 不进入当前关联事务；对应阶段记入 skipped_stages，不计为已完成。
...
- 参数组固定 32 槽，当前关联运行/FF/PI/导航参数；最初 old 快照不随候选修订改变。RAM 临时值期间暂停自动/显式持久化，实际验证内环、Heading、转驱和路径后统一保存。
...
| `RoverMotionRequest.msg` | 冻结包络内 SOURCE_CALIBRATION 开环/真实 PI 闭环，纵向非负，不伪装成 Navigation |
...
本轮执行位置 `/mnt/e/freertos/h743_freertos`（Windows 别名 `E:\freertos\H743_FreeRTOS`）。保留共享工作树的传输、串口等其他改动；没有提交、推送、刷机或连接板卡。

本轮使用 Linux 原生 Python/Make/Arm GCC，构建目录 `build-linux/`。正式参数、消息、Metadata、日志合同与必要签名均由正常构建链生成；未调用独立 verify 目标。建议工具通过内存/临时输入冒烟，不新增测试文件、测试框架或测试基础设施。

```bash
make PYTHON=python3 BUILD_DIR=build-linux dima_rover
python3 tools/calibration_suggest.py --messages messages.txt --params rover.params
```

2026-09-14 最终原生构建 exit 0，耗时 205.38 s；22 项工具输入冒烟通过，源码空白检查通过。完整制品大小/哈希见文末本轮记录，下面 Windows 表格仅保留历史证据。

### 历史 Windows 验收记录（不代表本轮源码）
...
| 顶档/部分覆盖/停车 | 缓慢提升且不越冻结包络；受限样本不得报告端点，停车须排空末端 slew |
| 前进与原地转向 | 负纵向请求拒绝；CW/CCW 轮端可反向，停车及 MOT_REV_DELAY 仍生效 |
...
| `MOT_THR_MAX` | 入场冻结 | 唯一输出包络（手动对等），开/闭环统一遵守 |
...
## 2026-09-14 修订：冻结包络、前进激励与磁-油门补偿（ADR 0005）

- 退役 RO_CAL_THR_MAX/RO_CAL_TURN_MAX，校准请求与闭环纵向采用 [0,E]、转向 [-1,1]、轮端 [-E,E]。看门狗保留归一化/新鲜度和 100 ms 收紧，不再设置校准专属幅值限制。
- 删除校准 REVERSE/FULL、倒退证据与许可，FWD→CW→CCW；运行速度/FF 不读取缺失的倒退剖面，Manual 共用整形仍需完整使用域证据。
- 无运动按时间继续升档；只有到达 0.995E 后连续 8 s 无运动才报告 DRIVE_ENVELOPE。端点必须来自前进顶档可用样本的后端已应用输出，一次报告。
- PI 仅公式法；速度 FF 需小于 min(0.35,E)，角速度 FF 需小于 min(0.30,E)，不可行不生成降级候选。
- 原始磁样本与后端已应用输出对齐（过去 100 ms 内、32 槽历史，丢帧/反向/未知输出拒绝）；高输出直线段去程/返程分别中心化，至少 100 个样本、每段至少 30、有效输出跨度 ≥0.15E，逐轴回归并检查方差及 0.08 G 残差。最终 `K_body=R*S*K_sensor`。
- 基础磁校准确认并保存后，独立 COMMIT_MAG_MOT 事务应用五项 CAL_MAG_MOT_KX/KY/KZ/ID/GEN、确认前端代次再保存。源码 YAML 的 Firmware Observations 分组生成只读/持久化策略；QGC 可见且协议拒绝任何外部写入，包括写零。普通产品常量的快照保护保持独立。
- 相关磁校正/安装角改参在参数锁内清有效 ID，回滚最后恢复完整观测快照；GEN 保留最近代次，成功提交递增，溢出拒绝。ID=0 表示失效，零 K 可以是有效学习结果。
- `VehicleMagnetometer` 只在两轮均非负的前进工况，以同时刻实际纵向均值应用 `B-K*u`；原地转向和倒退不套用直线模型。干扰率为 `100*norm(K_body)*E/WMM场强`，未知 -1，允许超过 100%，超过 30% 提示硬件整改。
- STAGE_MAG 与 STAGE_MAG_MOT 分别报告。存在磁力计但补偿样本不足/确认或保存失败时为 PARTIAL，已确认回滚后可继续其他整定；回滚无法确认仍锁存禁 Arm。
- 建议工具只读解析 QGC 五列或两列参数和最近 [autocal] 会话。缺失信息为未知，不代填默认值，不建议缩小停车申报、放宽保护或猜测 PI。

### 本轮本机验收

执行 `make PYTHON=python3 BUILD_DIR=build-linux dima_rover` 和临时输入工具冒烟；不新增测试文件、测试基础设施或独立 verify 目标。最终构建 exit 0（205.38 s），工具 22 项输入冒烟通过，输入哈希未改变；301 项参数、5 项补偿观测值均生成 readOnly，两个退役参数不存在。QGC 只读显示/拒写、断电恢复/重新校准失效、弱动力起步和实际磁补偿效果均为板端待验收项。
...
应用 Flash 607876/782336 B（77.7%），DTCM 静态 60832 B，D2 数据 200992 B；向量地址 0x08040400。链接映射已包含 MagMotorOutputHistory.o 和 VehicleMagnetometerCompensation.o。

| 制品 | 字节数 | SHA-256 |
|---|---:|---|
| `build-linux/H743_FreeRTOS.elf` | 11064204 | `86f140a5d3d14ff52d42df52f173c325d013823850638cf217ce66b770fa6ae9` |
| `build-linux/H743_FreeRTOS.bin` | 607876 | `d72b0f7e0c7d6ed5a67ef2dcf8306d92ae51ee2ba663d968a2e6c1e629e6828e` |
| `build-linux/H743_FreeRTOS_signed.bin` | 609051 | `286b6ee0a9e4e7a19fde2ffc255cf1cea117b8239c5a606245c6be228cb4b72d` |
| `build-linux/H743_FreeRTOS_factory.hex` | 1562894 | `faf06d0f5f920accf85d8022f9dd5adae67ee1a40f2b24e2a4396bde2df87f66` |

此记录证明本机正式生成、编译、链接及正常签名产物；本轮未执行独立 ELF/签名复验。QGC UI、重启持久化和车辆行为仍需板端验收。

## 2026-09-14 状态机闭合修复

- 磁事务 Done/Failed 先同步 expected_set_count，bootstrap 回滚后正常进入后续整定。
- 响应剖面改为 PROBE→RAMP→SETTLE→COLLECT→BRAKE：先得到可运动下界，六档覆盖有效区间，再做一档非零下降；到位后稳定 1 s、采样至少 10 点且覆盖 1.2 s 才换档。到顶后 8 s 无可观测运动才分流 DRIVE_ENVELOPE。
- 每个响应运动最多 90 s，子窗口按请求/MOT/末端 slew 中最慢者加 8 s 余量计算；全会话仍为 600 s，最晚在 585 s 开始该运动的停车流程。
- CW/CCW 入场检查完整杆臂转动范围，不再调用五米直线判据。正反向全局电机整形/SLEW 试改分支退出本模式，保留参数并报告 skipped_stages。
- 内环验证计数 0..2 为速度、3..5 为 CW、6..8 为 CCW；到 9 后进入 Heading，禁止重复启动越界角速度验证。
- 周期日志有 session 和证据快照；重复终态幂等，重连后可恢复端点、磁干扰和补偿状态。
- SUCCESS 只表示适用项目完成，支持项目证据不足仍为 PARTIAL。详细主流程、子状态、事务和授权见 [状态机文档](AUTO_CALIBRATION_STATE_MACHINE_ZH.md)。

本次修复后的最终构建与工具验收记录在状态机文档末尾；上文制品哈希保留为修复前历史记录。
