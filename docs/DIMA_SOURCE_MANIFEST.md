...
本清单记录上游版本、原始路径、本地映射和修改摘要。

仓库不再引入文件名包含 `license` 或 `licence` 的独立文件（不区分大小写）。涉及上游快照闭包变更时，使用 `tools/generation/source_manifest.py` 重新生成校验清单，禁止手写文件列表或摘要。
...
| 基础多航点 Position/Waypoint/PivotTurn | PX4 v1.17.0；APM 仅作行为参考 | `Dima/rover/modes/auto/AutoMode.*`、`Dima/lib/rover/`、`Dima/modules/mission/` | 9 | RUNTIME INTEGRATED / REVERSE EXCLUDED / WINDOWS BUILD VERIFIED / BOARD PENDING |
...
| PX4 Parameter Autosave 与 Runtime cache | `Dima/middleware/parameters/` | 对齐 PX4 v1.17.0 autosave.cpp / parameters.cpp：300 ms 合并、保存开始间隔至少 2 s、失败最多重试 3 次，一次编码并连续保存；ParameterService/Autosave 位于独立 `wq:storage`，不得以 SD 同步 I/O 阻塞 MAVLink；`Param<T>` 构造无 Core 副作用，每次 start bind，每次 shutdown 清 ready/used/unsaved/value cache、callback 和动态 Layer | ADAPTED / RUNTIME LIFECYCLE |
...
| Parameter FlashFS 与 Mission SD/RAM 行为 | `Dima/middleware/parameters/flashfs.*`、`FileStorage.*`、`Dima/modules/mission/`、`Dima/platform/api/AtomicFileStore.hpp` | Parameter 保持 FlashFS 主存储与 SD 镜像；payload 对齐 PX4 flashfs32.c 连续编程，底层保留一次 cache/ECC 回读，最终 commit 与加载 CRC 不变；Mission 仅使用独立 SD 文件域或非持久 RAM，不再持有 FlashFS。SD 完整快照采用临时写入、同步、回读验证和主备轮换；无可用 SD 时发布完整 RAM 任务并明确提示 volatile。任务 Flash 正文、版本标记和 legacy 恢复均不存在 | ADAPTED / SD-RAM MISSION STORAGE / BOARD PENDING |
| Rover Parameter 定义 | `Dima/middleware/parameters/definitions/` | Rover 参数继续由权威 YAML 进入正式 Parameter/Metadata 生成链；SYS_DM_BACKEND 已移除，任务后端按 SD 可用性自动选择，不提供替代参数或别名 | AUTHORITATIVE YAML / GENERATED |
| `platforms/common/include/px4_platform_common/log.h`、`platforms/common/px4_log.cpp`、`src/modules/logger/{logger,log_writer_file,messages}.*`、`platforms/common/uORB/uORBMessageFields.*` | `Dima/middleware/logging/`、`Dima/middleware/uORB/uORBMessageFields.*`、`Dima/modules/logging/{LogService,SdLogWriter,LogWriter}.*` | 保留 PX4 日志宏和 SourcePolicy；`messages.h` 逐字同步，`uORBMessageFields.*` 保留完整格式组流式实现，仅裁退产品无消费者的查找/逐字段/缓冲长度接口。普通/RAW 日志进入深度 8 的 `mavlink_log` 并有界转为 STATUSTEXT；Topic producer 自动遍历生成 catalog，按 PX4 写 header/Flag Bits、`F/P/Q/A/D/L/S/O`，固定 64 KiB SPSC consumer 在 `wq:storage` 以最大 8192-byte 分片写入并每 1 s 同步。新介质/文件 generation 从 header 重建；无卡/介质错误只停止 SD 副本，Event Ring 与实时文本继续。产品不引入无消费者的 PX4 Mission/MAVLink ULog backend、profile/rate 参数或 console/perf/events metadata | ADAPTED / PRODUCT TOPIC ULOG / WINDOWS BUILD VERIFIED / BOARD PENDING |
...
| EKF2 `module.yaml`/`params_*.yaml` 与 Estimator/vehicle `.msg` | `Dima/middleware/parameters/definitions/module_ekf2*.yaml`、`module_sensor_params.yaml`、`Dima/messages/schemas/` | 参数名称、类型、默认值、reboot_required、Topic alias/ID/hash/registry 均由既有正式 Parameter/uORB 工具派生；没有手写派生目录、消息列表或 `EKF2_EN`。产品 schema 可退役无消费者的 FIFO Topic，并在保留上游主 Topic、顺序和完整 payload 的前提下裁剪别名；固定上游参考快照保持原样 | AUTHORITATIVE YAML/MSG / GENERATED CONTRACT |
...
| PX4 Mission 事务与 Rover 固定任务执行 | `src/modules/mavlink/mavlink_mission.cpp` → `Dima/modules/mission/`、`Dima/modules/mavlink/MavlinkMission.*` | 固定 64 项 staging/active；单项完成槽推进上传序列，完整 SD 保存或 RAM 发布后才发送最终 ACK。SD/RAM 介质选择是本地产品策略，任务内容 ID 和帧仍复用正式 MAVLink codec；清空是合法空快照，持久入口与 AUTO 运行进度分离。热插卡同步不更换现役 RAM 任务，不写任务 Flash | ADAPTED / MISSION PROTOCOL / SD-RAM STORAGE / BOARD PENDING |
...
| `src/modules/dataman/dataman.cpp`；`src/modules/mavlink/mavlink_mission.cpp` | `Dima/modules/mavlink/MavlinkMission.hpp/.cpp`、`Dima/modules/mission/` | 保留上传、顺序请求、回读、清空、设置当前和最终 ACK 协议。SYS_DM_BACKEND 从权威 YAML 删除，有可用 SD 时原子保存完整快照，无卡时仅保留 RAM；不读写旧任务 Flash。对照 PX4 v1.17.0 与锁定 QGC 的 MISSION_ITEM_INT frame 规范化和最终重传语义不变 | ADAPTED / MISSION_INT / SD-RAM / BOARD PENDING |
...
| 适配边界 | 上游 header 原样保留；项目配置和 `format_to`/`vformat_to` wrapper 位于 `Dima/lib/format/`；USB stdout/setvbuf 能力保持不变 |
...
| 既有原子参数/maintenance/autosave 暂停 | `CalibrationParameters::revise_float/apply_revisions/finalize_provisional` 提供同组候选更新与最终提交，旧无人调用的数组替换接口已退役；保留最初快照、消费者代次确认和跨 Arm 保存暂停。旧零增益回滚确认不要求导航重新就绪 |
...
本地校准只删除明确无消费者的旧事务入口、响应辅助接口和无用跟踪，保留真实 ARX/SLEW/applied 输入、幅值/一致性门禁及独立安全检查。临时归档基线、符号消失/合并证据、前后制品与并发 IMU 改动的统计边界记录在 `CODE_SIZE_REDUCTION_ZH.md`，不按源文件行数推算 Flash，也不把静态占用当成实板高水位。

第二轮沿用同一 Logger YAML/uORB/ULog 权威链，只调整本地生成表的存储表示：相同采样策略共享只读表，每个 Topic 保留全部 Profile 索引；名称/ID 和回放注册不再保留无人读取的冗余字段。producer 合并重复的采样拷贝选择和发送结果分支，固定频率、逐 generation 回放、停止边沿、背压与 dropout 合同保持。未改上游快照、ULog wire、参数或消息定义；核对和制品证据续记于同一减重文档。
...
第五轮按授权共享本地 `CalibrationIdentification` 的模型评估实体：本轮未修改 `ArxRls` 更新内核，也未减少六个延迟模型；样本数预检查及 getter 顺序仍在薄模板适配中，评估公式、候选选择与 PI 逻辑保持原顺序。Metadata 仅将现有主机 XZ 压缩改为 preset 9 + extreme，原参数/执行器 JSON 与 wire 契约不变，CRC 和嵌入数据继续由正式工具生成；分步制品及并发变动的证据边界见 `CODE_SIZE_REDUCTION_ZH.md` 第 9 节。
...
### Manual 电机行为复核（2026-09-14）

按既定 ArduPilot `3f2e4763accb` 的 `Rover/mode.cpp` pilot normalization、`libraries/AR_Motors/AP_MotorsUGV.cpp` 的 skid mixing、MIN/EXPO 与独立反转延时、`SRV_Channel.cpp` 的中位两侧 PWM 映射核对。H743 自有实现补齐 Manual 两轴同比缩放、非对称域转向范围和 MIN 后 EXPO 顺序；不限制急转时内轮反转，不将单侧换向扩展为双侧等待。保留 H743 的最终 E 包络、导航零纵向停车、Arm ramp 和数值零阈值，参数继续用既有归一化单位。没有向源码树导入 APM 实现。

使用临时二进制和内存输入对照参考函数：26010 组中 26004 组数值一致（最大误差 5.96e-7），6 组只有参考计算约 2.98e-8 的零附近残差被本地零阈值抑制；没有非预期差异。另有 5292 组本地 E 包络与严格零输出检查通过。未新增测试文件、框架或测试基础设施；这些结果不证明现场 RC 输入、实际 PWM、电调中位或车辆轨迹。
