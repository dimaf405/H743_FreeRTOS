...
| Parameter FlashFS 与 Mission SD/RAM 行为 | `Dima/middleware/parameters/flashfs.*`、`FileStorage.*`、`Dima/modules/mission/`、`Dima/platform/api/AtomicFileStore.hpp` | Parameter 保持 FlashFS 主存储与 SD 镜像；Mission 仅使用独立 SD 文件域或非持久 RAM，不再持有 FlashFS。SD 完整快照采用临时写入、同步、回读验证和主备轮换；无可用 SD 时发布完整 RAM 任务并明确提示 volatile。任务 Flash 正文、版本标记和 legacy 恢复均不存在 | ADAPTED / SD-RAM MISSION STORAGE / BOARD PENDING |
