...
默认没有 `SERIALx_FUNCTION=3`，不预占数传 UART。GPS/SBUS 原默认配置保持，SERIAL5 继续为空号。选择实际接线的空闲串口，设置 `SERIALx_FUNCTION=3 (MAVLink)` 即可；参数写入顺序不限。将新端口设为 MAVLink 会通过参数原子事务关闭旧 MAVLink owner，始终只有一个 UART 数传 owner；映射写入成功即提示 reboot required，端口接管在下次启动执行（`SERIALx_BAUD/FUNCTION` 均为 reboot_required 参数，QGC 自动弹出重启提示）。`SERIALx_BAUD` 显式为非零值时直接以该速率运行；保持 `0=Auto` 时帧格式仍为 8N1，波特率由固件按常见数传速率逐档自动探测（9600..921600 共八档，每档观察 4 s，窗口内收到不少于 2 帧完整 MAVLink 帧即锁定；锁定速率不回写参数，重启后重新探测）。参数仍只有一份权威目录，USB 和 UART 的参数读取快照及下载游标分别保存。
...
串口映射为**重启生效合同**（对齐 ArduPilot/PX4）：`SERIALx_BAUD/FUNCTION` 标记 reboot_required，写入时由参数层原子事务校验并迁移 owner（始终单一 owner，非法候选回滚），随后提示 reboot required；端口接管、波特率应用与 Auto 扫描全部在下次启动执行。运行期没有串口维护票据、UART 排空状态机或热重配/回滚路径，USB 始终保留配置入口。

普通重启 ACK 返回请求来源链路，提交成功与物理发送完成分开记录。ACK 必须具有正常完成代数变化、无发送失败且队列排空的证据；错误恢复中止发送后的空闲不算送达。未取得证据而超过推导期限时取消重启并报告，防止低波特率截断 ACK。复位还要求当前非 volatile 参数全部保存，保存等待期限为 10 s，存储 I/O 仍由 `wq:storage` 执行；保存未完成或暂停保存、连接失效、等待期间解锁/安全状态过期均取消本次请求。`param1=3` 的 MCUboot Recovery 只允许 USB，并采用相同参数保存条件，固件上传继续使用 USB。
...
| 自身切换 | 通过 UART 修改自身 BAUD/迁移到另一空闲端口：写入返回成功并提示 reboot required，重启后按新映射启用；非法 owner 候选在写入时即被拒绝且不迁移。 |
...
## 2026-09-10 Auto 扫描风暴修复（IWDG 复位）

板端实测修改 GPS 串口参数触发板级复位掉线，根因是 Auto 扫描与既有错误处理的交互：扫描驻留在错误候选档位时，错速噪声使 UART 累计错误代数（`error_generation`）每个调度周期都变化，`MavlinkEndpoint::Run` 的故障路径因此以 100 Hz 执行 `discard_rx()+reset_link()`——每次重建协议会话、向 `wq:storage` 调度日志 reader 复位并重发心跳/AUTOPILOT_VERSION；维护重配的 quiesce 快照比较在同一噪声下必然失配，导致维护每秒重试。lp_default/storage 饱和使 BootHealth 健康代数停止推进，appMain 按 IWDG 合同停止喂狗，2048 ms 后看门狗复位。

修复（最小引入版，端点零改动）：扫描未锁定期间**不运行端点 `Run()`**——由 `MavlinkService` 用本地 parser（独立 msg/status，逐轮 256 B 预算）直接读取传输层并累计完整帧作为锁定证据；不派发、不应答、不发送心跳，错速噪声的累计错误因此既进不了端点故障路径，也进不了 quiesce 快照判据（未锁定扫描端点无在途业务，跳过 Run 与错误比较）。锁定时调用端点公开的 `reset_link()` 一次重建干净协议上下文，随后恢复正常 Run 与故障语义；锁定后端点被关闭（维护/故障）时从当前档位重扫；显式 BAUD 与停止路径仅复位扫描状态。扫描期错误计数仍经 `SerialMavlinkTransport::service()` 的 1 Hz 汇总观察。全部变更收敛于 `MavlinkManager.cpp` 与 `MavlinkService.hpp` 的 4 个扫描状态成员，`MavlinkEndpoint` 与既有 USB/显式 BAUD 路径保持逐字节等价。

Linux 原生 `make -j4 NO_COLOR=1 dima_rover` 通过（Total 1,227,196 B，68.2%）。板端复测项：Auto 模式下修改 GPS 串口参数不再复位、维护事务收敛、扫描锁定后链路可用。

## 2026-09-10 Info 一次性与传输/应用解耦修订

板端实测暴露 `Sensor status` 摘要以 ~60 Hz 刷屏并导致 QGC 掉线。根因是两层语义错配，本次按 PX4 v1.17 原则修正：

1. **非错误 Info 一次性**：传感器摘要原本绑定在链路建立事件上，持续线路错误使建立事件每调度周期重演。现改为服务级一次性合同——任一端点首次链路就绪输出一条，本 Runtime 不再重复；健康变化由既有边沿消息（`Magnetometer detected`/`data timeout`/IMU fault/recovered）承担，持续健康经 `SYS_STATUS` 位掩码周期流表达，STATUSTEXT 只承载一次性事件。早期 30 s 限频 workaround 已移除。
2. **字节级错误只撤半帧**：`error_generation` 变化（丢字节/framing 错）不再触发全量 `reset_link()`，只 `discard_rx()`；错帧由 parser 校验失败自然丢弃重同步，协议会话（参数流、Mission token、日志租约、epoch、链路就绪状态）保持。全量复位仅保留给真实端口关闭边沿（`!ready && transport_was_ready_`：停止/扫描换档/维护 quiesce）。串口 rxerr 从此与 CAN 设备（DroneCAN 磁力计）状态上报完全解耦；1 Hz `UART MAVLink drop=...` Warning 保留为线路错误唯一周期性诊断。

Linux 原生 `make -j4 NO_COLOR=1 dima_rover` 通过后记录最终数值。板端复测项：错速/悬空 UART 下无 STATUSTEXT 洪水且摘要整个上电周期仅一条；拔/插 CAN 磁力计各出一条边沿消息；UART 持续错误期间 `SYS_STATUS` 流速率与内容不变。

## 2026-09-10 串口配置机制大修（重启生效合同）

板端实测确认运行期热重配链（维护票据 → UART 排空 → 应用/回滚 → 恢复）在噪声线路、扫描换档与多消费者启停下不可收敛，先后引发 IWDG 复位、1.4 s 重试循环、参数修改失败与 QGC 掉线。本次按 ArduPilot/PX4 机制大修：

- **重启生效**：`SERIALx_BAUD/FUNCTION` 全部标记 `reboot_required: true`（与 EKF2 参数同一元数据合同，QGC 自动弹出重启提示）。写入时仅由 `set_serial_function` 参数原子事务校验/迁移 owner 并提示 `serial mapping updated; reboot required to apply`；端口接管、波特率应用与 Auto 扫描全部在启动时执行。
- **删除运行期热重配链**：`SerialConfig::reconfigure/rollback_configuration/pending_configuration_valid/configuration_signature`、`MavlinkService::prepare_serial_reconfigure/resume_serial/serial_reconfigure_failed/apply_serial_configuration` 与 quiesce 状态机、`ApplicationContext` 的 `SerialReconfigurePhase` 状态机/维护票据/应用函数全部移除；`MavlinkEndpoint::Run` 不再有 quiescing 入参。`drain_timeout_us` 保留用于普通重启 ACK 的物理完成期限。
- **接口收缩**：`SerialPortAssignments` 移除 `telemetry_configuration_valid`；`SerialConfig` 不再保存 active/previous 配置快照与回滚波特率数组。
- **保留 Auto 波特率扫描**：BAUD=Auto 的八档扫描、每档 4 s 窗口、≥2 帧锁定证据、扫描期不派发不应答的全部合同不变。

Linux 原生 `make -j4 NO_COLOR=1 dima_rover` 与 `parameter-metadata-verify` 通过后记录最终数值。板端复测项：改串口参数不再复位/失败（写入返回成功并提示重启）；重启后新映射生效；Auto 扫描与显式 BAUD 均正常建立链路。
