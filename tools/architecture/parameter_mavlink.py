"""校验 Dima YAML 参数闭包、上游派生产物与 MAVLink 生成边界。"""
...
def _scan_parameter_generation_contract(violations: list[Violation]) -> None:
    ...
    for path in input_paths:
        if path.suffix.lower() not in (".yaml", ".yml"):
            violations.append(Violation(
                path, 1, "R331", "parameter generator inputs must be module YAML"
            ))
        if not path.is_file():
            violations.append(Violation(
    ...
    try:
        ...
        violations.append(Violation(
            ...
            f"cannot read generated upstream parameter header: {error}",
        ))
    else:
        expected_public_header = "#pragma once\n" + raw_header.replace(
            "namespace px4", "namespace dima::parameter_catalog"
        )
        # 只允许 namespace 常量表改用 C++17 单一定义链接属性；仍逐字核对
        # 整个上游输出的其余内容，不能为减重手工重写参数项或省略任何字段。
        expected_public_header = re.sub(
            r"(?m)^static constexpr ", "inline constexpr ", expected_public_header
        )
        if ("This file is autogenerated from parameters.xml" not in raw_header or
                raw_header.count("namespace px4") != 2 or
                public_header != expected_public_header):
            violations.append(Violation(
                public_header_path, 1, "R331",
                "public Dima parameter header must be the mechanical namespace/inline-table adaptation",
            ))
    ...
    for literal in required_make_literals:
        if literal not in project_text:
            violations.append(Violation(
                project_path, 1, "R331",
                f"official Dima YAML build wiring is missing: {literal}",
            ))
    old_make = re.search(
