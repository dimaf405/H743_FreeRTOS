...
LOGGER_GENERATED_DIR := $(BUILD_DIR)/generated/logging
...
DIMA_LOGGER_GENERATED_INCLUDES := -I$(LOGGER_GENERATED_DIR)
...
DIMA_HEATSHRINK_INCLUDES := \
	-IMiddlewares/Third_Party/px4_heatshrink
...
LOGGER_CONTRACT_GENERATOR := tools/logging/generate_logger_contract.py
LOGGER_POLICY := Dima/modules/logging/logger_topics.yaml
LOGGER_PARAMETER_SOURCE := \
	Dima/middleware/parameters/definitions/module_logger.yaml
LOGGER_ULOG_SOURCE := \
	$(UORB_UPSTREAM_ROOT)/src/modules/logger/messages.h
LOGGER_CONTRACT_STAMP := $(LOGGER_GENERATED_DIR)/.generated.json
LOGGER_CONTRACT_HEADER := $(LOGGER_GENERATED_DIR)/logger_contract.hpp
LOGGER_SIDECAR_HEADER := $(LOGGER_GENERATED_DIR)/logger_sidecar.hpp
LOGGER_ULOG_HEADER := $(LOGGER_GENERATED_DIR)/ulog_messages.hpp
LOGGER_GENERATED_OUTPUTS := \
	$(LOGGER_CONTRACT_STAMP) \
	$(LOGGER_CONTRACT_HEADER) \
	$(LOGGER_SIDECAR_HEADER) \
	$(LOGGER_ULOG_HEADER)
LOGGER_GENERATOR_DEPS := \
	$(LOGGER_CONTRACT_GENERATOR) $(LOGGER_POLICY) $(LOGGER_PARAMETER_SOURCE) \
	$(LOGGER_ULOG_SOURCE) $(UORB_SOURCE_MANIFEST) $(MESSAGE_SCHEMAS)
...
$(LOGGER_CONTRACT_STAMP): make/project.mk $(LOGGER_GENERATOR_DEPS) \
		...
		$(HOST_TOOLS_STAMP)
	$(DIMA_PROGRESS_RUN) --label LOGGER_GEN --target "$@" \
		--display "$(LOGGER_GENERATED_DIR)" -- \
		env PYTHONDONTWRITEBYTECODE=1 PYTHONUTF8=1 \
			PYTHONPATH="$(HOST_PYTHON_DIR)" \
			$(PYTHON) $(LOGGER_CONTRACT_GENERATOR) \
			--policy $(LOGGER_POLICY) \
			--schemas $(MESSAGE_SCHEMA_DIR) \
			--uorb-topics $(MESSAGE_GENERATED_DIR)/topics/uORBTopics.hpp \
			--parameters $(PARAMETER_GENERATED_DIR)/parameters.json \
			--parameter-source $(LOGGER_PARAMETER_SOURCE) \
...
.PHONY: logger-generated logger-generated-verify
logger-generated: $(LOGGER_GENERATED_OUTPUTS)

logger-generated-verify: $(LOGGER_GENERATED_OUTPUTS)
	$(DIMA_PROGRESS_RUN) --label LOG_VERIFY --target "$@" -- \
		env PYTHONDONTWRITEBYTECODE=1 PYTHONUTF8=1 \
			PYTHONPATH="$(HOST_PYTHON_DIR)" \
			$(PYTHON) $(LOGGER_CONTRACT_GENERATOR) \
			--policy $(LOGGER_POLICY) \
			...
			--ulog-source $(LOGGER_ULOG_SOURCE) \
			--source-manifest $(UORB_SOURCE_MANIFEST) \
			--output $(LOGGER_GENERATED_DIR) --verify
...
DIMA_HEATSHRINK_C_OBJECTS := \
	$(addprefix $(BUILD_DIR)/,$(DIMA_HEATSHRINK_C_SOURCES:.c=.o))
...
$(DIMA_COMPOSITION_OBJECTS): DIMA_PRIVATE_INCLUDES += \
	...
	$(DIMA_ADAPTER_INCLUDES) \
	$(DIMA_MAVLINK_GENERATED_INCLUDES) \
	$(DIMA_LOGGER_GENERATED_INCLUDES)
...
$(DIMA_UORB_MESSAGE_FIELDS_OBJECT): DIMA_PRIVATE_INCLUDES += \
	...
	$(DIMA_HEATSHRINK_INCLUDES)
$(DIMA_LOGGING_MODULE_OBJECTS): DIMA_PRIVATE_INCLUDES += \
	$(DIMA_HEATSHRINK_INCLUDES) $(DIMA_LOGGER_GENERATED_INCLUDES)
...
$(DIMA_HEATSHRINK_C_OBJECTS): DIMA_PRIVATE_DEFS := \
	-DHEATSHRINK_DYNAMIC_ALLOC=0
$(DIMA_HEATSHRINK_C_OBJECTS): DIMA_PRIVATE_INCLUDES := \
	$(DIMA_HEATSHRINK_INCLUDES)
...
$(DIMA_FATFS_FREERTOS_CXX_OBJECTS): DIMA_PRIVATE_INCLUDES += \
	$(DIMA_LOGGER_GENERATED_INCLUDES)
