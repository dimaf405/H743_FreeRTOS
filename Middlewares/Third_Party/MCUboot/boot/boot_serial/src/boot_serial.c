    ...
    IMAGES_ITER(image_index) {
        for (slot = 0; slot < BOOT_NUM_SLOTS; slot++) {
            ...
            if (rc) {
                ...
                {
                    const uint32_t upload_image_id = image_index * 2 + slot + 1;

                    /* Only advertise IDs accepted by the platform policy.
                     * This target deliberately forbids direct writes to the
                     * running primary slot. */
                    if (flash_area_id_from_direct_image(upload_image_id) ==
                        flash_area_get_id(fap)) {
                        ok = zcbor_tstr_put_lit(cbor_state, "upload_image_id") &&
                             zcbor_uint32_put(cbor_state, upload_image_id);
                    }
                }
#else
                if (slot == 1) {
